import { configuredOfficialServerUrl } from '@/services/appConfig'
import { vaultItemMatchesSearch } from '@/services/search'
import { vaultItemMatchesListFilters } from './searchFilters'
import type { VaultStoreState } from './state'
import { uniqueById } from './syncObjects'

export function countVisibleVaultItems(state: Pick<VaultStoreState, 'vaults' | 'vaultItemCounts'>): number {
  const visibleVaultIds = new Set(
    uniqueById(state.vaults)
      .filter((vault) => !vault.sync.deletedAt)
      .map((vault) => vault.id),
  )

  return Object.entries(state.vaultItemCounts).reduce(
    (total, [vaultId, count]) => total + (visibleVaultIds.has(vaultId) ? count : 0),
    0,
  )
}

export const vaultGetters = {
  hasUsers: (state: VaultStoreState) => state.users.length > 0,
  activeUser: (state: VaultStoreState) => {
    return state.users.find((user) => user.id === state.activeUserId) ?? state.users[0] ?? null
  },
  needsUserSetup: (state: VaultStoreState) => {
    const activeUser = state.users.find((user) => user.id === state.activeUserId) ?? state.users[0] ?? null
    return state.users.length === 0 || !activeUser?.crypto
  },
  visibleVaults: (state: VaultStoreState) => uniqueById(state.vaults).filter((vault) => !vault.sync.deletedAt),
  visibleItems: (state: VaultStoreState) => uniqueById(state.items).filter((item) => !item.sync.deletedAt),
  writableVaults: (state: VaultStoreState) => uniqueById(state.vaults).filter((vault) => !vault.sync.deletedAt),
  visibleAttachments: (state: VaultStoreState) => uniqueById(state.attachments).filter((attachment) => !attachment.sync.deletedAt),
  selectedItem: (state: VaultStoreState) => {
    const items = uniqueById(state.items)
    return items.find((item) => !item.sync.deletedAt && item.id === state.selectedItemId)
      ?? items.find((item) => !item.sync.deletedAt)
      ?? null
  },
  selectedItemAttachments: (state: VaultStoreState) => {
    const items = uniqueById(state.items)
    const item = items.find((candidate) => !candidate.sync.deletedAt && candidate.id === state.selectedItemId)
      ?? items.find((candidate) => !candidate.sync.deletedAt)
    if (!item) return []
    return uniqueById(state.attachments)
      .filter((attachment) => !attachment.sync.deletedAt && item.attachmentIds.includes(attachment.id))
  },
  filteredItems: (state: VaultStoreState) => {
    return uniqueById(state.items).filter((item) => vaultItemMatchesListFilters(item, state.attachments, {
      selectedVaultId: state.selectedVaultId,
      selectedType: state.selectedType,
      query: state.query
    }))
  },
  quickResults: (state: VaultStoreState) => {
    return uniqueById(state.items)
      .filter((item) => !item.sync.deletedAt && vaultItemMatchesSearch(item, state.query, state.attachments))
  },
  syncConnected: (state: VaultStoreState) => Boolean(state.settings.sync.deviceId && state.settings.sync.syncSpaceId),
  syncLocalChangeCount: (state: VaultStoreState) => {
    return [
      ...uniqueById(state.vaults),
      ...uniqueById(state.items),
      ...uniqueById(state.attachments)
    ].filter((object) =>
      object.sync.state === 'dirty' || object.sync.state === 'pending'
    ).length
  },
  syncConflictCount: (state: VaultStoreState) => {
    return [
      ...uniqueById(state.vaults),
      ...uniqueById(state.items),
      ...uniqueById(state.attachments)
    ].filter((object) => object.sync.state === 'conflicted').length
  },
  syncHostLabel: (state: VaultStoreState) => {
    const serverUrl = state.settings.sync.mode === 'official'
      ? configuredOfficialServerUrl()
      : state.settings.sync.serverUrl
    return serverUrl.replace(/^https?:\/\//i, '').replace(/\/$/, '')
  },
  vaultCount: (state: VaultStoreState) => {
    return (vaultId: string | 'all') => {
      if (vaultId === 'all') return countVisibleVaultItems(state)
      return state.vaultItemCounts[vaultId] ?? 0
    }
  }
}
