import type { VaultAttachment, VaultItem, VaultItemField, VaultItemFieldKind, VaultItemType } from '@lockpass/core'
...
export function fieldLabel(t: Translate, kind: VaultItemFieldKind, fallback = ''): string {
...
export function displayValue(field: VaultItemField, showSensitive: boolean): string {
...
export function detailFields(t: Translate, item: VaultItem): VaultItemField[] {
  ...
  return [...urlFields, ...item.fields.filter((field) => field.kind !== 'note')]
}
...
export function defaultFields(t: Translate, type: VaultItemType): VaultItemField[] {
...
export function attachmentKind(t: Translate, attachment: Pick<VaultAttachment, 'fileName' | 'mimeType'>): string {
...
export function attachmentIcon(attachment: Pick<VaultAttachment, 'fileName' | 'mimeType'>): string {
  const name = attachment.fileName.toLowerCase()
...
export function isImageAttachment(attachment: Pick<VaultAttachment, 'fileName' | 'mimeType'>): boolean {
  const name = attachment.fileName.toLowerCase()
  return (attachment.mimeType || '').startsWith('image/') || /\.(png|jpe?g|jfif|webp|gif|bmp|heic|avif|svg)$/.test(name)
}
