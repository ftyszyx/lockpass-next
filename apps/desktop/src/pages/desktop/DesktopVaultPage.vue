...
import { generatePassword, type VaultItem, type VaultItemField, type VaultItemType } from '@lockpass/core'
...
import { startDeepLinkListener, subscribeToDeepLinks } from '@/services/deepLink'
...
import { defaultFields, detailFields, getInitials, makeField } from './formatters'
import type { DetailTab, DrawerName, ItemDraft, ManagementPageName, ModalName, OperationProgressState, PasswordOptions, ToastState, UserDraft, VaultDraft } from './types'
import { useColumnResize } from './useColumnResize'

const { t } = useI18n()
const vaultStore = useVaultStore()
const { filteredItems, selectedItem, selectedItemAttachments, visibleAttachments, visibleItems, writableVaults } = storeToRefs(vaultStore)
...
const removingUserFromDevice = ref(false)
...
const operationProgress = reactive<OperationProgressState>({ visible: false, title: '', body: '' })
...
const { mainGridStyle, resizingTarget, startColumnResize, onResizeHandleKeydown } = useColumnResize(vaultStore, mainGrid)
...
const hasLegacyImport = computed(() => Object.keys(vaultStore.legacyPayloads).length > 0)
const pendingDeleteVault = computed(() => vaultStore.vaults.find((vault) => vault.id === pendingDeleteVaultId.value) ?? null)
const pendingDeleteVaultItemCount = computed(() => pendingDeleteVaultId.value ? vaultStore.vaultCount(pendingDeleteVaultId.value) : 0)
...
async function initializeVaultPage(): Promise<void> {
  ...
  if (vaultStore.hasUsers && !vaultStore.needsUserSetup && !vaultStore.unlocked) {
...
async function removeActiveUserFromDevice(): Promise<void> {
  removingUserFromDevice.value = true
  try {
    ...
    activeModal.value = vaultStore.needsUserSetup ? null : 'lock'
    if (removed) {
      showToast(t('toast.userRemovedFromDevice', { name: removed.displayName }))
    }
  } catch (error) {
    showToast(error instanceof Error ? error.message : String(error))
  } finally {
    removingUserFromDevice.value = false
  }
}

function requestDeleteVault(vaultId: string): void {
  const vault = vaultStore.vaults.find((candidate) => candidate.id === vaultId && !candidate.sync.deletedAt)
  if (!vault) return
...
async function confirmDeleteVault(): Promise<void> {
  ...
  try {
    ...
    pendingDeleteVaultId.value = null
    activeModal.value = null
    showToast(t('toast.vaultDeleted', { name: result.vaultName, count: result.itemCount }))
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    showToast(message === 'vault-delete-last' ? t('vault.deleteLastBlocked') : message)
  } finally {
...
function openNewItem(type: VaultItemType = 'login'): void {
  ...
  itemError.value = ''
  itemDraft.type = type
  itemDraft.vaultId = vaultStore.selectedVaultId === 'all' ? writableVaults.value[0]?.id ?? '' : vaultStore.selectedVaultId
  itemDraft.title = ''
  itemDraft.notes = ''
...
function openEditItem(): void {
  ...
  itemDraft.notes = item.notes || item.fields.find((field) => field.kind === 'note')?.value || ''
...
function validateItemDraft(): string {
  if (!itemDraft.vaultId || !itemDraft.title.trim()) return t('editor.requiredMissing')

  if (itemDraft.type === 'login' && !fieldValue('password')) return t('editor.requiredMissing')
  if (itemDraft.type === 'payment-card' && !fieldValue('card-number')) return t('editor.requiredMissing')
  if (itemDraft.type === 'secure-note' && !itemDraft.notes.trim()) return t('editor.requiredMissing')
  return ''
}

function fieldValue(kind: VaultItemField['kind']): string {
  return itemDraft.fields.find((field) => field.kind === kind)?.value.trim() ?? ''
}
  ...
  try {
    ...
    const drafts = await Promise.all(
      files.map(async (file) => {
        ...
        const saved = await saveAttachmentFile(activeUserId, id, file, sessionKey.vaultKey, sessionKey.keyId)
        return {
          id,
          fileName: file.name,
...
async function createBackup(): Promise<void> {
  await runBackupTask({ title: t('progress.backupExportTitle'), body: t('progress.backupExportBody') }, async () => {
    const backup = await vaultStore.exportBackupPackage()
    const result = await downloadTextFile(backupFileName(), JSON.stringify(backup, null, 2))
    savedBackupResult.value = result
  })
}

async function restoreBackup(file: File): Promise<void> {
  await runBackupTask({ title: t('progress.backupRestoreTitle'), body: t('progress.backupRestoreBody') }, async () => {
...
async function importCsv(file: File): Promise<void> {
  await runBackupTask({ title: t('progress.csvImportTitle'), body: t('progress.csvImportBody') }, async () => {
    ...
    showToast(t('backup.importSuccess', { count: result.imported, vault: result.vaultName }))
  })
}

async function exportCsv(): Promise<void> {
  await runBackupTask({ title: t('progress.csvExportTitle'), body: t('progress.csvExportBody') }, async () => {
    const result = await downloadTextFile(csvFileName(), await vaultStore.exportCsvText(), 'text/csv')
    showToast(textFileSavedToast('backup.csvExportSuccess', result))
  })
}

async function importLegacyBackup(payload: { file: File; password: string }): Promise<void> {
  await runBackupTask({ title: t('progress.legacyImportTitle'), body: t('progress.legacyImportBody') }, async () => {
    ...
    const imported = await importExternalVaults(result.vaults, t('backup.legacyVaultName'))
    showToast(t('backup.legacyImportSuccess', { count: imported.imported, vaults: imported.vaults }))
  })
}

async function importExternalItems(items: ExternalImportItem[], vaultName: string): Promise<{ imported: number; vaultName: string }> {
...
async function importExternalVaults(vaults: ExternalImportVault[], fallbackVaultName: string): Promise<{ imported: number; vaults: number }> {
  if (!vaultStore.unlocked) throw new Error('syncLocked')
  const result = await vaultStore.importExternalVaults(vaults, fallbackVaultName)
  if (result.imported === 0 && result.vaults === 0) throw new Error('backup-empty-import')
  return { imported: result.imported, vaults: result.vaults }
}

async function runBackupTask(progress: Pick<OperationProgressState, 'title' | 'body'>, task: () => Promise<void>): Promise<void> {
...
function backupErrorToast(error: unknown): string {
  ...
  if (message === 'legacy-password-invalid') return t('backup.legacyPasswordInvalid')
  if (message === 'backup-empty-import') return t('backup.emptyImport')
  return message || t('backup.failed')
}

function textFileSavedToast(messageKey: 'backup.exportSuccess' | 'backup.csvExportSuccess', result: TextFileSaveResult): string {
  if (result.target === 'tauri') {
    return t('backup.exportSavedTo', { path: result.path })
  }
  return t('backup.exportDownloadedToBrowser', { fileName: result.fileName, message: t(messageKey) })
}
...
async function openSavedBackupDirectory(path: string): Promise<void> {
  try {
    ...
    showToast(opened ? t('backup.directoryOpened') : t('backup.browserDirectoryOpenUnsupported'))
  } catch (error) {
    showToast(error instanceof Error ? error.message : t('backup.directoryOpenFailed'))
  }
}
...
function removeDraftAttachment(id: string): void {
  itemDraft.attachments = itemDraft.attachments.filter((attachment) => attachment.id !== id)
}
...
async function copyValue(value: string, message = t('toast.copied')): Promise<void> {
  try {
...
function showOperationProgress(progress: Pick<OperationProgressState, 'title' | 'body'>): number {
...
async function runWithOperationProgress<T>(progress: Pick<OperationProgressState, 'title' | 'body'>, task: () => Promise<T>): Promise<T> {
...
async function runSidebarSyncNow(): Promise<void> {
  ...
  try {
    const result = await runWithOperationProgress({ title: t('progress.syncTitle'), body: t('progress.syncBody') }, () => vaultStore.runSync())
...
async function useSavedRecoveryKey(): Promise<void> {
  ...
  try {
    ...
    const sessionUnlock = await perf.measure('store.unlockActiveUserWithSessionCache', () =>
    ...
    const result = await perf.measure('secureStorage.loadRecoveryKey', () => vaultStore.loadSavedRecoveryKeyForActiveUser())
    if (result.status === 'loaded') {
      const valid = await perf.measure('store.unlockActiveUser', () => vaultStore.unlockActiveUser(unlockPassword.value, result.recoveryKey))
      if (!valid) {
...
async function unlockApp(): Promise<void> {
  ...
  await nextFrame()
  try {
    const valid = await perf.measure('store.unlockActiveUser', () => vaultStore.unlockActiveUser(password, recoveryKey))
    if (!valid) {
      authError.value = t('user.wrongUnlockSecret')
      return
    }
...
async function saveRecoveryKeyAfterManualUnlock(recoveryKey: string): Promise<void> {
  const perf = createPerfTrace('saveRecoveryKeyAfterManualUnlock')
  try {
    const storageStatus = await perf.measure('secureStorage.saveRecoveryKey', () => vaultStore.saveVerifiedRecoveryKeyForActiveUser(recoveryKey))
    ...
    perf.done({ failed: true, error: error instanceof Error ? error.message : String(error) })
...
async function loadRecoveryKeyForAnotherDevice(): Promise<void> {
  ...
  try {
    ...
    revealRecoveryKeyIssue.value = result.status === 'unsupported' ? 'unsupported' : 'missing'
...
async function saveRecoveryKeyToDevice(recoveryKey: string): Promise<void> {
  ...
  try {
    ...
    revealRecoveryKeyIssue.value = storageStatus === 'unsupported' ? 'unsupported' : 'missing'
    revealError.value = storageStatus === 'unsupported'
      ? t('settings.recoveryKeyUnsupported')
...
async function changeSecuritySettings(settings: Partial<DesktopSecuritySettings>): Promise<void> {
  await vaultStore.saveSecuritySettings(settings)
  showToast(t('toast.settingsSaved'))
}

async function changeShortcut(payload: { scope: ShortcutScope; action: ShortcutAction; shortcut: string }): Promise<void> {
...
async function openDesktopLogDir(): Promise<void> {
  try {
    ...
    showToast(path ? t('settings.openLogDirSuccess') : t('settings.openLogDirBrowserPreview'))
  } catch {
...
function scheduleAutoLock(): void {
  cancelAutoLock()
  if (!vaultStore.unlocked || !vaultStore.settings.security.autoLockOnLimit) return
...
<template>
  <div v-if="initializing && !vaultStore.hydrated && !vaultStore.storageError" class="grid h-screen min-h-[560px] place-items-center bg-[#f7f8fa] p-6 text-slate-950">
  ...
  <div v-else-if="vaultStore.storageError && !vaultStore.hydrated" class="grid h-screen min-h-[560px] place-items-center bg-[#f7f8fa] p-6 text-slate-950">
    <section class="grid w-full max-w-xl gap-5 rounded-lg border border-rose-200 bg-white p-6 shadow-xl shadow-rose-950/5">
      <span class="grid size-12 place-items-center rounded-lg bg-rose-50 text-rose-700">
        <TriangleAlert class="size-6" />
      </span>
      <div class="grid gap-2">
        <h1 class="text-2xl font-black">{{ t('app.storageError') }}</h1>
        <p class="text-sm leading-6 text-slate-600">{{ t('app.storageErrorBody') }}</p>
      </div>
      <div class="grid gap-1 rounded-lg border border-rose-100 bg-rose-50 px-3 py-2">
        <span class="text-xs font-bold text-rose-700">{{ t('app.storageErrorDetail') }}</span>
        <code class="break-words font-mono text-xs text-rose-900">{{ vaultStore.storageError }}</code>
      </div>
      <button class="primary-button justify-self-start" type="button" :disabled="initializing" @click="initializeVaultPage">
  ...
  <div v-else class="grid h-screen min-h-[680px] grid-rows-[56px_minmax(0,1fr)] bg-[#f7f8fa] text-slate-950">
    ...
    >
      <ManagementPage
        ...
        @remove-active-user="openRemoveUserModal"
    ...
    <UserSetupModal
      v-if="(vaultStore.hydrated && vaultStore.needsUserSetup) || activeModal === 'user' || generatedRecoveryKey"
      :draft="userDraft"
    ...
    <RemoveUserModal
      ...
      :removing="removingUserFromDevice"
      @close="activeModal = null"
      @confirm="removeActiveUserFromDevice"
    />
    ...
    <LockOverlay
      v-if="activeModal === 'lock' || (!activeModal && vaultStore.hydrated && vaultStore.hasUsers && !vaultStore.needsUserSetup && !vaultStore.unlocked)"
