...
const emit = defineEmits<{
  ...
  changeShortcut: [
    payload: { scope: ShortcutScope; action: ShortcutAction; shortcut: string },
  ];
  resetShortcuts: [];
  openLogDir: [];
...
const pageTitle = computed(
  () =>
    pages.value.find((page) => page.name === props.activePage)?.label ??
    t("management.title"),
);
const visibleShortcutActions = computed(() =>
  shortcutScope.value === "global"
    ? GLOBAL_SHORTCUT_ACTIONS
    : INTERNAL_SHORTCUT_ACTIONS,
);
const shortcutsAreDefault = computed(() =>
  shortcutSettingsEqual(
...
watch(
  ...
  (page) => {
    ...
    if (page === "shortcuts" && shortcutScope.value === "global")
      void checkVisibleGlobalShortcuts();
    if (page !== "shortcuts") stopCapturingShortcut();
  },
  { immediate: true },
);

watch(shortcutScope, (scope) => {
  if (props.activePage === "shortcuts" && scope === "global")
    void checkVisibleGlobalShortcuts();
});
...
watch(
  ...
  () => {
    if (props.activePage === "shortcuts" && shortcutScope.value === "global")
      void checkVisibleGlobalShortcuts();
  },
);
...
async function loadSystemInfo(): Promise<void> {
  ...
  try {
    ...
    appDataDir.value = "";
    appDataDirError.value =
      error instanceof Error ? error.message : t("system.appDataDirFailed");
  } finally {
...
async function openLocalStorageDir(): Promise<void> {
  try {
    ...
    emit(
      "systemToast",
      path
        ? t("system.appDataDirOpenSuccess")
        : t("system.appDataDirBrowserPreview"),
    );
    if (path) appDataDir.value = path;
  } catch (error) {
    emit(
      "systemToast",
      error instanceof Error ? error.message : t("system.appDataDirOpenFailed"),
    );
  }
}
...
async function loadDesktopLogs(): Promise<void> {
  ...
  try {
    ...
    logText.value = "";
    logLoadError.value =
      error instanceof Error ? error.message : t("logs.failed");
  } finally {
...
async function syncStartOnLoginState(): Promise<void> {
  ...
  try {
    ...
    if (
      enabled !== null &&
      enabled !== vaultStore.settings.security.startOnLogin
    ) {
...
async function changeStartOnLogin(event: Event): Promise<void> {
  ...
  try {
    ...
    if (applied === null)
      emit("systemToast", t("settings.startOnLoginUnsupported"));
  } catch {
...
function changeAutoLockDelay(event: Event): void {
  const autoLockDelaySeconds = Number(
...
function submitLegacyImport(): void {
  ...
  emit("importLegacyBackup", {
    file: legacySelectedFile.value,
...
function shortcutValue(scope: ShortcutScope, action: ShortcutAction): string {
  return normalizeShortcut(
    String(vaultStore.settings.shortcuts[scope][action as never] ?? ""),
  );
}
...
function shortcutCapturePlaceholder(
  scope: ShortcutScope,
  action: ShortcutAction,
): string {
...
function isCapturingShortcut(
  ...
  return (
    capturingShortcut.value?.scope === scope &&
    capturingShortcut.value.action === action
  );
}
...
async function recordShortcut(
  ...
  if (!isShortcutValid(shortcut, scope)) {
    setShortcutError(
      ...
      action,
      scope === "global"
        ? t("shortcuts.globalShortcutInvalid")
        : t("shortcuts.internalShortcutInvalid"),
    );
    return;
  }
  ...
  <section class="grid min-h-0 grid-cols-[240px_minmax(0,1fr)] bg-[#f7f8fa]">
    ...
    <div class="grid min-h-0 grid-rows-[64px_minmax(0,1fr)]">
      ...
      <main class="min-h-0 overflow-auto p-6">
        <div v-if="activePage === 'backup'" class="grid max-w-3xl gap-4">
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <span
                ><strong>{{ t("drawer.createBackup") }}</strong
                ><small>{{ t("backup.exportHint") }}</small></span
              >
            </button>
            <button
              class="action-card"
              type="button"
              :disabled="backupBusy"
              @click="openFilePicker(backupFileInput)"
            >
              <span class="action-icon"><ArchiveRestore class="size-4" /></span>
              <span
                ><strong>{{ t("drawer.restoreBackup") }}</strong
                ><small>{{ t("backup.restoreHint") }}</small></span
              >
            </button>
              ...
              <span class="grid min-w-0 gap-2">
                ...
                <span class="flex flex-wrap gap-2">
                  <button
                    ...
                    type="button"
                    :disabled="backupBusy"
                    @click="openFilePicker(csvFileInput)"
                  >
                    {{ t("backup.importCsv") }}
                  </button>
                  <button
          ...
          <input
            ...
            class="hidden"
            type="file"
            accept=".lpbackup,.json,.lpbackup.json,application/json"
          ...
          />
          <input
            ref="legacyBackupInput"
            class="hidden"
            type="file"
            accept=".zip,application/zip,application/x-zip-compressed"
            @change="onLegacyBackupSelected"
          />
        </div>
        ...
        <div v-else-if="activePage === 'settings'" class="grid max-w-3xl gap-6">
          ...
          >
            <label class="form-label">
              ...
              <select
                class="form-input"
                :value="vaultStore.settings.locale"
                @change="
                  emit(
                    'changeLocale',
          ...
          >
            ...
            <label class="form-label">
              ...
              <select
                class="form-input"
                :value="vaultStore.settings.logging.level"
                @change="
                  emit(
                    'changeLogLevel',
                    ($event.target as HTMLSelectElement)
                      .value as DesktopLogLevel,
                  )
                "
              >
                <option v-for="level in logLevels" :key="level" :value="level">
                  {{
                    t(
                      `settings.logLevel${level[0].toUpperCase()}${level.slice(1)}`,
                    )
                  }}
                </option>
              </select>
          ...
          <section class="rounded-lg border border-slate-200 bg-white p-5">
            ...
            <label v-if="!webRuntime" class="setting-row">
              <span>
                ...
                <small>{{
                  startOnLoginSupported
                    ? t("settings.startOnLoginHint")
                    : t("settings.startOnLoginUnsupported")
                }}</small>
              </span>
              <input
                type="checkbox"
            ...
            <label class="setting-row">
              ...
              <select
                class="form-input max-w-44"
                :value="
                  vaultStore.settings.security.autoLockOnLimit
                    ? vaultStore.settings.security.autoLockDelaySeconds
                    : 0
                "
                @change="changeAutoLockDelay"
              >
                <option
                  v-for="seconds in autoLockDelayOptions"
                  :key="seconds"
                  :value="seconds"
                >
            ...
            <label class="setting-row"
              ><span
                ><strong>{{ t("settings.clearClipboard") }}</strong
                ><small>{{ t("settings.clearClipboardHint") }}</small></span
        ...
        <div
          v-else-if="activePage === 'shortcuts'"
        ...
        >
          ...
          >
            ...
            <div
              class="inline-grid grid-cols-2 rounded-lg border border-slate-200 bg-slate-50 p-1"
            >
              <button
            ...
            <div class="overflow-hidden rounded-lg border border-slate-200">
              <div
                class="grid grid-cols-[minmax(160px,220px)_minmax(220px,1fr)] gap-3 border-b border-slate-200 bg-slate-50 px-4 py-2 text-xs font-black text-slate-500"
              >
              ...
              >
                ...
                <div class="grid gap-1">
                  <input
                    ...
                    :value="
                      isCapturingShortcut(shortcutScope, action)
                        ? ''
                        : shortcutValue(shortcutScope, action)
                    "
                    :placeholder="
                      shortcutCapturePlaceholder(shortcutScope, action)
                    "
                    readonly
                  ...
                  />
                  <span
                    v-if="shortcutErrors[shortcutKey(shortcutScope, action)]"
                  ...
                  <span
                    v-else-if="
                      shortcutChecking[shortcutKey(shortcutScope, action)]
                    "
                    class="text-xs text-slate-400"
                    >{{ t("shortcuts.checkingConflict") }}</span
                  >
        ...
        <div v-else-if="activePage === 'logs'" class="grid max-w-5xl gap-4">
          ...
          >
            ...
            <div
              class="overflow-hidden rounded-lg border border-slate-200 bg-slate-950"
            >
              <pre
                v-if="logText"
                class="max-h-[560px] min-h-[320px] overflow-auto p-4 font-mono text-xs leading-5 text-slate-100"
                >{{ logText }}</pre
              >
              <div
                v-else
                class="grid min-h-[320px] place-items-center p-6 text-center text-sm text-slate-300"
              >
                <span v-if="logLoading">{{ t("logs.loading") }}</span>
                <span v-else-if="logLoadError" class="text-rose-200">{{
                  logLoadError
                }}</span>
            ...
            >
              ...
              >
                ...
                <button
                  ...
                  @click="emit('copyValue', appDataDir)"
                >
                  <Copy class="size-4" />
                </button>
              </div>
              <span
                v-else-if="appDataDirLoading"
                class="text-sm text-slate-400"
                >{{ t("system.loading") }}</span
              >
              <span v-else-if="appDataDirError" class="text-sm text-rose-700">{{
                appDataDirError
              }}</span>
              <span v-else class="text-sm text-slate-400">{{
                t("system.appDataDirBrowserPreview")
    ...
    <div
      v-if="legacyImportOpen"
      class="fixed inset-0 z-[80] grid place-items-center bg-slate-950/40"
    >
      <section
        class="w-[460px] max-w-[94vw] rounded-lg border border-slate-200 bg-white shadow-2xl"
      >
        <div
        ...
        <div v-if="!legacySelectedFile" class="grid gap-4 p-4">
          <div class="grid gap-2">
            <span
              class="grid size-10 place-items-center rounded-lg bg-teal-50 text-teal-700"
            >
              <DatabaseBackup class="size-5" />
            </span>
            <strong class="text-lg text-slate-950">{{
              t("backup.legacyPickTitle")
            }}</strong>
            <p class="text-sm text-slate-500">
              {{ t("backup.legacyPickBody") }}
            </p>
          </div>
          <button
            ...
            type="button"
            :disabled="backupBusy"
            @click="openFilePicker(legacyBackupInput)"
          >
        ...
        <div v-else class="grid gap-4 p-4">
          ...
          <div class="grid gap-2">
            ...
            <strong class="text-lg text-slate-950">{{
              t("backup.legacyPasswordTitle")
            }}</strong>
            <p
              class="break-all rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600"
            >
