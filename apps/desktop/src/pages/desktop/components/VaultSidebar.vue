  ...
  <aside class="flex min-h-0 flex-col border-r border-slate-200 bg-[#f1f5f4]">
    <div class="relative px-3 py-3">
      ...
      <div v-if="userMenuOpen" class="absolute left-3 right-3 top-[66px] z-40 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xl">
        <div class="p-1">
          <button class="grid min-h-9 w-full grid-cols-[20px_minmax(0,1fr)] items-center gap-2 rounded-md px-2 text-left text-sm hover:bg-teal-50" @click="manageUsers">
            <Users class="size-4" />
          ...
          <button class="grid min-h-9 w-full grid-cols-[20px_minmax(0,1fr)] items-center gap-2 rounded-md px-2 text-left text-sm hover:bg-teal-50" @click="showRecoveryKey">
            <QrCode class="size-4" />
            <span class="truncate">{{ t('settings.setupAnotherDeviceTitle') }}</span>
          </button>
          <button class="grid min-h-9 w-full grid-cols-[20px_minmax(0,1fr)] items-center gap-2 rounded-md px-2 text-left text-sm hover:bg-teal-50" @click="openManagement">
