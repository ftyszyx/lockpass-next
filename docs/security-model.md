...
1. 明文只在桌面端解锁会话中短暂存在。
2. 主密码、安全密钥（`recoveryKey`）、vault key 和可解密密钥不上传服务器。
...
7. 备份包必须加密并认证，不允许把安全密钥（`recoveryKey`）或其他辅助密钥明文打包。
...
flowchart TD
  ...
  recoveryKey["安全密钥 recoveryKey<br/>客户端生成的高熵 Secret Key 类密钥<br/>离线保存；已信任设备默认进系统安全存储"]
  kdf["KDF 参数<br/>Argon2id、salt、iterations 等<br/>可以随密文保存"]
  unlockKey["unlockKey<br/>由主密码 + 安全密钥 + KDF 参数派生<br/>只在解锁会话内短暂存在"]
...
```text
# 安全密钥（recoveryKey）：辅助主密码派生 unlockKey，用户需要离线保存；已信任设备默认保存在系统安全存储。
...
2. 安全密钥（`recoveryKey`）必须是随机生成的高熵字节串，展示和输入时使用 base64url 或分组文本；不能把主密码和安全密钥做普通字符串拼接。
3. KDF 输入必须带域隔离标签和长度前缀，例如 `lockpass unlock v1`，避免不同用途复用同一输入。
4. Argon2id 参数必须随 `wrappedVaultKey` 保存；当前开发阶段客户端只接受本地定义的 KDF profile，避免同一开发库里混用多套解锁参数。
5. `wrappedVaultKey`、条目、附件索引和备份包都必须用 AEAD，加密时必须传入 AAD；面向同步和备份的对象 envelope，AAD 至少包含用途、对象类型、对象 id、vault id、schema version、key id 和相关 revision。桌面端本地整包 payload envelope 如果作为过渡格式存在，应绑定 `accountId` / `keyId` / `schemaVersion`；进入服务端保存或备份导出时必须拆成 per-object envelope。
...
## 安全密钥（recoveryKey）的作用和重要性

安全密钥（`recoveryKey`）是主密码之外的第二份用户秘密，定位接近 1Password 的 Secret Key，用来提高 `unlockKey` 的强度。它不负责直接加密条目，也不直接加密 `vaultKey`；它参与派生 `unlockKey`，再由 `unlockKey` 解开 `wrappedVaultKey`。

安全密钥不能找回、重置或绕过主密码，也不能单独解锁保险库。用户必须同时拥有正确的主密码和安全密钥，才能从头完成完整解锁或新设备恢复。产品文案必须明确这一点，避免让用户误以为“安全密钥”是忘记主密码后的恢复码。
...
安全密钥（`recoveryKey`）的保管规则。以下规则同时描述当前本机能力和目标态跨设备恢复能力；凡涉及从服务端下载 `wrappedVaultKey` 并在新设备恢复保险库的流程，当前仍属于协议预留/待完成：

1. 服务端不能保存安全密钥明文，也不能通过同步接口、备份 manifest 或日志获得安全密钥。
2. 安全密钥不进入普通 app data，例如 SQLite、IndexedDB、配置 JSON、同步缓存或未加密导出目录。
3. 备份包不能包含安全密钥明文；如果未来支持把系统安全存储内容纳入平台备份，也必须依赖系统安全存储自身的保护边界，不能把明文落入 LockPass 备份包。
4. 每个已信任设备默认把安全密钥保存到系统安全存储：Windows Credential Manager、macOS Keychain、Linux Secret Service。应用本地数据库只保存是否已信任、key handle 或状态标记，不保存明文。
5. 已解锁旧设备可以在用户再次验证主密码或通过系统认证后显示安全密钥，也可以展示用于迁移到新设备的二维码。二维码内容等价于安全密钥，必须按同等敏感级别处理，并在短时间后自动隐藏。当前桌面端可展示/保存安全密钥；二维码和新设备迁移仍以原型或待完成能力为准。
6. 目标态新设备恢复时，用户扫描/输入安全密钥，并输入主密码；客户端本地派生 `unlockKey`，再用它解开 `wrappedVaultKey`。当前实现不能让文案承诺该跨设备恢复闭环已经可用。
7. 如果用户选择“不信任本设备”或“不保存到系统安全存储”，该设备必须要求用户离线保存安全密钥，例如纸质记录、离线 U 盘或安全备份；该设备之后不能 reveal、导出或生成迁移二维码，只能在每次需要时让用户手动输入/扫描。
8. 系统安全存储主要防普通本地数据库泄露、服务端泄露和备份文件泄露；它不防已攻陷终端、已解锁会话、恶意软件、键盘记录器或能调用用户态凭据 API 的攻击者。
9. 如果用户丢失所有已信任设备，也丢失离线保存的安全密钥，服务端无法帮助恢复 vault 内容。

## 受信任设备快速解锁

完整解锁路径仍然是主密码 + 安全密钥派生 `unlockKey`，再解开 `wrappedVaultKey`。这条路径用于首次解锁、修改主密码、重建本地快速解锁材料，以及系统安全存储不可用或校验失败时的回退；新设备恢复使用同一目标态路径，但当前仍属于同步协议预留/待完成。
...
| 普通 app data | `deviceWrappedVaultKey`、快速解锁元数据 | `deviceWrappedVaultKey` 是密文；元数据只保存 `accountId`、`deviceId`、`vaultId`、`keyId`、`deviceKeyId`、schema version、创建时间、最后使用时间、是否要求系统认证等非秘密字段。 |
...
deviceUnlockKeyHandle = platformProtectedStore.wrapAndStore(
  ...
  account = accountId + ":" + deviceId + ":" + deviceKeyId,
...
如果平台没有能强制用户存在性校验的能力，客户端不得建立或展示免主密码快速解锁；只能要求用户输入主密码，并可从系统安全存储读取已保存的安全密钥来完成完整解锁。如果系统认证失败、`deviceWrappedVaultKey` 解不开、`keyId` 已轮换、设备被本地移除，或快速解锁材料超过本地策略有效期，客户端必须清理该设备的快速解锁材料，并回退到主密码 + 安全密钥完整解锁。

平台要求：

1. Windows：免主密码快速解锁使用 CNG/KSP 强保护本机私钥包裹 `deviceUnlockKey`，私钥使用时必须触发 Windows 系统认证或强保护 UI；如果运行环境不能提供该能力，或只能使用 Windows Credential Manager，则不得启用免主密码快速解锁，只能保存安全密钥并要求用户输入主密码。
...
4. 浏览器预览不支持快速解锁，只能走主密码 + 安全密钥完整解锁。

策略边界：

1. 快速解锁不替代安全密钥。用户仍必须离线保存安全密钥；丢失安全密钥后，服务端仍无法帮助恢复。
2. 快速解锁只降低本机日常解锁成本，不改变服务端零知识边界；PC 未锁屏时仍然依赖应用自动锁定和系统锁屏，产品不能把“受信任设备”描述成能抵抗同一桌面会话里的旁人操作。
3. 主密码修改、安全密钥轮换、`vaultKey` 轮换、设备撤销、从此设备移除用户时，必须删除旧 `deviceUnlockKey` 和 `deviceWrappedVaultKey`，并在下一次完整解锁后重新建立。
4. 快速解锁开启、关闭、重建和失败都必须记录非敏感审计事件；日志禁止记录主密码、安全密钥、`deviceUnlockKey`、`vaultKey` 和任何明文条目。
5. 产品文案不能把快速解锁描述成“无需主密码即可恢复账号”。它只能解锁当前受信任设备上已存在的本地密文。

## 服务器账号与保存流程

产品语义上，不把“同步”作为用户需要理解和主动操作的概念。用户进入桌面端后应登录服务器账号：可以选择 LockPass 官方托管，也可以填写自己的服务器地址。登录成功后，服务器账号负责多设备保存、设备管理和密文分发；主密码和安全密钥仍只负责本机解锁。

服务器账号创建在网站上完成，目标流程见 `web-account-creation-flow.md`。第一版优先使用邮箱验证码验证邮箱所有权，不把保险库主密码作为服务器长期密码凭据。网站可以引导用户设置主密码、生成 Secret Key、备份 Secret Key，但这些步骤必须在客户端本地完成；服务器只能得到邮箱、显示名称、设备绑定信息、`wrappedVaultKey`、KDF 参数和密文对象，不能得到主密码、Secret Key、`unlockKey` 或 `vaultKey` 明文。

用户可见流程必须简化为：

1. 打开应用并解锁本机保险库。
2. 如果尚未登录服务器账号，引导用户登录官方托管或自建服务器。
3. 联网时，新增、编辑、删除条目必须立即保存到服务器。
4. 断网时，条目仍先保存到本机密文库，并标记为待保存。
5. 一旦恢复联网、窗口回到前台或用户手动检查保存状态，客户端必须先检查本机是否有修改，再立即上传待保存修改并拉取服务器变化。

界面只展示“未登录服务器 / 等待保存到服务器 / 已保存到服务器 / 有冲突”等状态，不要求用户理解同步队列、cursor、revision 或事件流。

服务器是默认同步目的地和多设备分发节点，但不是明文数据源。服务端保存的是 per-object 密文、revision、cursor 和必要同步元数据；主密码、安全密钥、`unlockKey`、`vaultKey` 和条目明文都不能上传。
...
if trustThisDevice:
  ...
  setupFastUnlock(vaultKey, accountId, deviceId, vaultId, keyId)
else:
...
不上传到服务端：主密码 + 安全密钥明文 + unlockKey + vaultKey 明文 + 明文条目
不进入普通 app data 或 LockPass 备份明文：安全密钥
```
...
新设备使用安全密钥（`recoveryKey`）恢复，当前为目标态协议预留，不能作为已完成用户能力宣传：

```text
从服务端下载：wrappedVaultKey + 密文条目 + KDF 参数
用户输入：主密码 + 安全密钥
安全密钥来源：旧设备重新验证后显示文本/二维码，或用户离线保存的安全密钥
...
第一次启动必须登录服务器账号，并设置或输入用于解锁保险库的主密码。主密码不能明文保存，也不能写入日志、同步请求或备份 manifest。

本机缓存按服务器账号隔离：每个服务器账号有独立的 `accountId`、主密码 KDF 参数、保险库、条目、附件集合、设备 token 和待保存修改。切换服务器账号时必须锁定当前会话并清理当前账号的会话密钥，目标账号只有完成自己的解锁校验后才能进入。

桌面端锁定分为软锁和硬锁。软锁用于应用仍在当前进程内、用户主动点锁定或短时间离开后的日常场景：可以在内存中保留 `vaultKey` 和轻量主密码校验值，用户仍必须输入主密码，校验成功后用内存中的 `vaultKey` 解密本地 payload，从而避免每次都运行 Argon2id。软锁缓存不得写入磁盘、不得上传、不得跨应用重启保留；切换服务器账号、从此设备移除账号、安全密钥或 `vaultKey` 轮换、保存状态异常、应用退出、进程崩溃恢复、系统注销或后续接入的明确“硬锁”动作必须清理该缓存。冷启动和缓存缺失时仍必须走主密码 + 安全密钥 + Argon2id 的完整解锁路径；新设备恢复也使用同一目标态路径，但跨设备闭环当前仍待完成。
...
桌面端本地当前阶段如果仍有整包格式，可以把单个服务器账号的 `vaults`、`items`、`attachments` 索引作为一个本机 envelope 保存，附件 blob 仍单独 envelope 加密。这个本地整包格式只作为过渡本机持久化；一旦进入服务端保存或备份导出，条目、附件索引和备份 manifest 必须使用 per-object/per-package envelope，并绑定 `vaultId`、`revision` 和外层同步元数据。
...
| 类别 | 服务端可明文保存 | 必须加密 |
| --- | --- | --- |
| 账号和设备 | account id、登录凭据哈希、device id、设备名称、设备状态、token hash、最后同步时间 | 主密码、安全密钥（`recoveryKey`）、`unlockKey`、`vaultKey` |
...
本机移除服务器账号、关闭受信任设备、设备 token 撤销或用户主动关闭快速解锁时，客户端必须删除该设备的 `deviceUnlockKey` 和 `deviceWrappedVaultKey`。这只阻止该设备后续使用快速解锁；如果设备仍持有安全密钥或曾经缓存过明文/会话密钥，仍不等同于密码学撤销。
...
| 软锁后旁人操作 | 软锁仍要求输入主密码；进程内缓存只跳过 Argon2id，不提供免密码访问；硬锁、切换服务器账号和退出应用必须清理缓存 |
...
| 日志泄露 | 日志禁止记录主密码、token、Secret Key、密文解密结果 |
...
`wrappedVaultKey` 的 `aad.purpose` 必须是 `wrap-vault-key-v1`，并且 `aad` 必须包含 `keyId`、`kdfVersion` 和 `schemaVersion`。如果 `vaultKey` 只服务于单个 vault，还必须包含 `vaultId`；如果桌面端本地当前阶段使用账号级 `vaultKey`，则必须包含 `accountId` 并由本地账号 envelope 绑定账号范围。

`deviceWrappedVaultKey` 的 `aad.purpose` 必须是 `device-wrap-vault-key-v1`，并且 `aad` 必须包含 `accountId`、`deviceId`、`vaultId`、`keyId`、`deviceKeyId` 和 `schemaVersion`。快速解锁时必须用本地元数据重新构造 expected AAD，不能信任 envelope 内部自带字段绕过绑定检查。
...
当前开发阶段只接受上述 profile：`memoryKiB = 32768`、`iterations = 2`、`parallelism = 1`。如需调整参数，开发库需要删除后重新创建账号；正式发布前再引入参数迁移策略。
