...
5. Web 创建账号完成后，当前浏览器生成不可导出的 AES-256-GCM 设备密钥，把 Secret Key 加密保存到 IndexedDB，并成为受信任浏览器；设备密钥和 Secret Key 明文都不能写入 `localStorage`。
6. 受信任浏览器存储不等价于系统安全存储。控制当前浏览器 Profile、当前 Origin 执行环境或已解锁 OS 用户会话的攻击者，仍可能调用设备密钥解密 Secret Key。
